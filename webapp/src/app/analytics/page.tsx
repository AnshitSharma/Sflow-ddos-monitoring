...
export default function AnalyticsPage() {
  ...
  const { data: asnRows } = useSWR(['asn', mode], () =>
    queryInstant(mode === 'live' ? Q.topAsnSrc(8) : Q.topAsnSrcAvg(8)), { refreshInterval: 15000 });
  const { data: ccRows } = useSWR(['cc', mode], () =>
    queryInstant(mode === 'live' ? Q.topCountrySrc(8) : Q.topCountrySrcAvg(8)), { refreshInterval: 15000 });
  ...
  const topCountry: TopRow[] = ccRows?.length  ? vectorToTopRows(ccRows, 'src')  : mock?.topCountry ?? [];
  const topProto: TopRow[]   = mock?.topProto ?? [];

  const totalBps = topCountry.reduce((a, b) => a + b.bps, 0);
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-edge bg-surface p-0.5">
          {[{ k: 'live', l: 'Live' }, { k: '24h', l: 'Last 24h' }].map(o => (
            <button key={o.k} onClick={() => setMode(o.k as 'live' | '24h')}
              className={`px-3 py-1 rounded-md text-[12px] font-medium transition-all duration-150 ${mode === o.k ? 'bg-accent-soft text-accent shadow-[inset_0_0_0_1px_rgba(77,159,255,0.3)]' : 'text-ink-faint hover:text-ink hover:bg-raised'}`}>{o.l}</button>
          ))}
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <Panel icon={Icon.Globe} title="Top source ASNs" subtitle="by ingress bps · click to drill into live IPs">
          <BarList rows={topASN} kind="bps" limit={8} colorFn={() => THEME.inbound} onSelect={drillToFlows} />
        </Panel>
        <Panel icon={Icon.Globe} title="Top source countries" subtitle="by ingress bps · click to drill into live IPs">
