    ...
    <RangeContext.Provider value={{ rangeKey, setRangeKey, paused }}>
      <div className="flex h-screen w-screen overflow-hidden bg-canvas text-ink">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-14 shrink-0 border-b border-edge bg-canvas/80 backdrop-blur flex items-center gap-3 px-4">
            ...
            <div className="ml-auto flex items-center gap-2.5">
              <div className="hidden md:flex items-center gap-2 rounded-lg border border-edge bg-surface px-2.5 py-1.5">
                <span className="relative flex h-2 w-2">
                  {live && <span className="noc-live-dot absolute inline-flex h-full w-full rounded-full" style={{ background: THEME.ok }} />}
                  <span className="relative inline-flex rounded-full h-2 w-2" style={{ background: live ? THEME.ok : paused ? THEME.warn : THEME.textFaint }} />
                </span>
                <span className="text-[11px] font-mono uppercase tracking-wider" style={{ color: live ? THEME.ok : paused ? THEME.warn : THEME.textDim }}>
                  {paused ? 'Paused' : live ? 'Live' : 'Static'}
                </span>
